...
export async function POST(req: NextRequest) {
  try {
    ...
    if (c.code_type === 'vip') {
      ...
      //   修法: 用 SQL 端 NOW() + INTERVAL 算 expire_at, 同时用 AT TIME ZONE 'UTC' 强转
      //     - NOW() + INTERVAL 是 timestamp with time zone (UTC)
      //     - AT TIME ZONE 'Asia/Shanghai' 转成 wall clock, 当 timestamp without time zone 存
      //     - 跟 wall clock 一致, 读出 JS 看到 wall - 8h (Asia/Shanghai 减 8)
      //     - parseNeonTime 修正 + 8h 还原
