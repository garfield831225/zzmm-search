...
export async function GET(req: NextRequest) {
  ...
  try {
    const r = await sql`SELECT id, name, status, source, link IS NOT NULL as has_link, link_code IS NOT NULL as has_code, created_at, updated_at FROM xx_resources WHERE id = ${id}`;
