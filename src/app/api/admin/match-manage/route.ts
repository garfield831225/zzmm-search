  ...
  try {
    if (tab === 'matched') {
      const rows = await sql`
        ...
        WHERE r.tmdb_id IS NOT NULL AND r.tmdb_id != ''
        ${category ? sql`AND r.category = ${category}` : sql``}
        ORDER BY r.updated_at DESC
        LIMIT ${pageSize} OFFSET ${offset}
      ` as any[];
      const cnt = await sql`SELECT COUNT(*)::int as cnt FROM xx_resources WHERE tmdb_id IS NOT NULL AND tmdb_id != '' ${category ? sql`AND category = ${category}` : sql``}`.catch(() => [{cnt:0}]) as any[];
      ...
      const rows = await sql`
        ...
        LIMIT ${pageSize} OFFSET ${offset}
      ` as any[];
      const cnt = await sql`SELECT COUNT(*)::int as cnt FROM xx_resources WHERE (tmdb_id IS NULL OR tmdb_id = '') ${category ? sql`AND category = ${category}` : sql``}`.catch(() => [{cnt:0}]) as any[];
      return NextResponse.json({ tab, items: rows, total: cnt[0]?.cnt, page, pageSize });
    }
  } catch (e: any) {
