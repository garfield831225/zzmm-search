...
async function _GET(request: NextRequest) {
  ...
  const block1 = await sql(`
    ...
    SELECT m.tmdb_id, m.view_count, m.link_count, m.updated_at, m.sub_types,
           ...
           COALESCE(c.release_date, d.release_date::text, d.first_air_date::text, '1900-01-01') as sort_key
    FROM matched m
  ...
  const block3 = await sql(`
    SELECT tmdb_id, tmdb_type, title, original_title, poster_path, backdrop_path,
           ...
           COALESCE(release_date::text, first_air_date::text, '1900-01-01') as sort_key
