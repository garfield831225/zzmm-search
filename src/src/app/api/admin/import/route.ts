    ...
    for (let i = 0; i < items.length; i += BATCH) {
      ...
      try {
        for (const item of batch) {
          ...
          await sql`
            INSERT INTO xx_resources (name, link, link_code, source, category, size, type, tags, tmdb_id, imdb_id, status, valid_status, view_count, created_at, updated_at)
            VALUES (${name}, ${link}, ${linkCode}, ${source}, ${category}, ${size}, null, '{}', null, null, 'active', 'unchecked', 0, NOW(), NOW())
          `;
        }
...
export async function GET() {
  ...
  return NextResponse.json({
    total: parseInt((result as unknown as any[])[0]?.count || '0'),
